#!/usr/bin/env python3
"""
Test the new stateful PSAMGenerator API on the Q&A dataset

This uses the C-level stateful generator which maintains residuals across calls.
"""
...
def generate_with_stateful_generator(psam, question_tokens, vocab, residual_config, max_length=20):
    """Generate answer using stateful generator"""
    reverse_vocab = {v: k for k, v in vocab.items()}
    ...
    for step in range(max_length):
        pred_ids, scores, _, _, _ = generator.predict(generated, max_predictions=10)
...
def generate_baseline(psam, question_tokens, vocab, max_length=20):
    """Generate answer using baseline (no residuals)"""
    reverse_vocab = {v: k for k, v in vocab.items()}
    generated = question_tokens.copy()

    for _ in range(max_length):
        pred_ids, scores, _, _, _ = psam.predict(generated, max_predictions=10)
...
def main():
    print("╔════════════════════════════════════════════════════════════╗")
    print("║   Stateful PSAMGenerator - Full Q&A Test                  ║")
    print("╚════════════════════════════════════════════════════════════╝\n")
    ...
    for test_q in test_questions:
        ...
        np.random.seed(42)
        generated_baseline = generate_baseline(psam, q_tokens, vocab)
        ...
        # Test WITH stateful generator
        np.random.seed(42)
        generated_stateful = generate_with_stateful_generator(psam, q_tokens, vocab, residual_config)
        answer_stateful = decode_tokens(generated_stateful[len(q_tokens):], vocab)

        stateful_has_subject = subject and subject in answer_stateful.lower()
        results_stateful.append(stateful_has_subject)

        print(f"Stateful Gen:    {answer_stateful}")
        print(f"  Subject match: {'✅' if stateful_has_subject else '❌'}")

        # Highlight improvement
        if stateful_has_subject and not baseline_has_subject:
            print("  🎉 STATEFUL GENERATOR FIXED IT!")
        elif baseline_has_subject and not stateful_has_subject:
            print("  ⚠️  Stateful generator made it worse")
    ...
    stateful_correct = sum(results_stateful)
    total = len(test_questions)

    print(f"\nBaseline (no residuals):")
    print(f"  Correct subject: {baseline_correct}/{total} ({baseline_correct/total*100:.1f}%)")

    print(f"\nStateful Generator (with residuals):")
    print(f"  Correct subject: {stateful_correct}/{total} ({stateful_correct/total*100:.1f}%)")

    improvement = stateful_correct - baseline_correct
    print(f"\nImprovement: {improvement:+d} questions")

    if improvement > 0:
        print(f"🎉 Stateful generator improved accuracy by {improvement} questions!")
    elif improvement == 0:
        print("📊 Stateful generator maintained same accuracy (no regression)")
    else:
        print(f"⚠️  Stateful generator decreased accuracy by {abs(improvement)} questions")

    print("\n" + "="*70)
    print("💡 KEY ACHIEVEMENT:")
    print("="*70)
    print("• Implemented C-level stateful generator with persistent residual buffer")
    print("• Residuals now carry forward across generation steps")
    print("• This is the proper way to do deferred activation!")
    print("• Future tuning of lookahead/decay/blend parameters can improve results")
